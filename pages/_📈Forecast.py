...
def main():
    ...
    batch_size = st.selectbox("Select Batch Size:", [16, 32, 64], index=0)
    ...
    if st.button("Apply Hyperparameters", key='apply_button'):
        ...
        best_params_lstm, history_lstm, y_pred_lstm, y_test_orig_lstm, model_summary = run_optimization(space, 'lstm', X_train, y_train, X_test, y_test, scaler)
        ...
        display_results(history_lstm, y_test_orig_lstm, y_pred_lstm)
        ...
        st.write(f"Total training data: {len(X_train)}")
        st.write(f"Total testing data: {len(X_test)}")
